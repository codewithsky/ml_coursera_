...
% create a blank results matrix
  ...
  err_val = zeros(length(C_list), length(sigma_list));
  ...
  for i = 1:length(C_list)
      for j = 1: length(sigma_list)
          ...
          result(row,:) = [err_val(i,j), C_test, sigma_test];
          row = row + 1;
      end
  end

 % Sorting err_val in ascending order
  sorted_result = sortrows(result, 1);
  
 % C and sigma corresponding to min(err_val)
  C = sorted_result(1,2);
